...
// and therefor, we see only the last color(the change is almost immediate).
// We can achieve the same effect by incrementing the delay.
// Thats why, for rendering purposes, we should prefer the sequential version
async function lightShowParallel() {
	...
	const p3 = changeBodyColor('indigo', 3000);
	const p4 = changeBodyColor('violet', 4000);
	await p1;
	await p2;
	await p3;
	await p4;
